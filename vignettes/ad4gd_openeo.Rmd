...
```{r berlin_satellite_data, eval = FALSE}
openeo_con <- openeo::connect(host = "https://openeo.dataspace.copernicus.eu")
openeo::login(openeo_con)

lakes_selected <- lakes_malte
collection_id <- "SENTINEL2_L2A"

lakes_selected_query <- lapply(seq_len(nrow(lakes_selected)), function(i) {
  kwb.satellite::openeo_get_data(lakes = lakes_selected[i,],
...
job_ids <- sapply(seq_len(nrow(lakes_selected)),
                  function(i) lakes_selected_query[[i]]$job$id)
...
```{r bb_satellite_data, eval = FALSE}
openeo_con <- openeo::connect(host = "https://openeo.dataspace.copernicus.eu")
openeo::login(openeo_con)

lakes_selected <- lakes_bb_selected
collection_id <- "SENTINEL2_L2A"
