...
system.time(
lakes_01_centroid <- kwb.satellite::gee_get_data_for_years(
  years = 2018,
...
dat <- lakes_01_centroid$centroid_mean_2018$data$satellite_data[[1]] %>% 
  dplyr::bind_cols(
    lakes_01_centroid$centroid_mean_2018$data$satellite_metadata[[1]]
  )
...
saveRDS(lakes_malte_centroid, "lakes_malte_centroid.rds")

system.time(
lakes_malte_centroid_parallel <- kwb.satellite::gee_get_data_for_years_parallel(
  years = years,
  lakes = lakes_malte[1:7,],
  centroid = TRUE,
  spatial_fun = "mean",
  col_lakename = "SEE_NAME",
  debug_dir = exp_dir, 
...
saveRDS(lakes_malte_centroid_parallel,
        fs::path_join(c(exp_dir, "lakes_malte_centroid_parallel.rds"))
)
...
system.time(
lakes_centroid <- kwb.satellite::gee_get_data_for_years_parallel(
  years = years,
...
saveRDS(lakes_centroid, "lakes_centroid.rds")
...
saveRDS(lakes_polygon, "lakes_polygon.rds")
