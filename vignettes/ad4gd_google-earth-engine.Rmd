...
doParallel <- length(lakes_berlin) >= min_size_for_parallel && 
...
sat_data <- if (doParallel) {
     ...
     parallel_fun <- function(idx, geo) {
       lakes <- geo[idx,]
       print(lakes)
       reticulate::use_condaenv("ad4gd")
       rgee::ee_Initialize()
       kwb.satellite::gee_get_data_for_years(years = 2018, 
                                lakes = lakes,
                                #bands = NULL,
                                centroid = TRUE
       )
     ...
     parallel::clusterExport(cl = cl, varlist = c("lakes_berlin"))
       ...
       kwb.utils::catAndRun(
        ...
        expr = {
          ...
          parallel::parLapply(cl,
                              list(idx = 1:7, geo = lakes_berlin),
                              fun = function(inputs) {
                              parallel_fun(inputs$idx, inputs$geo)
                                })
