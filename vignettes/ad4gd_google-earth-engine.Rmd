...
shp_path <- system.file("extdata/berlin/Berlin_points.shp", package = "kwb.satellite")
#shp_path <- fs::path_abs("../inst/extdata/berlin/Berlin_points.shp")

lakes_malte <- sf::read_sf(shp_path) %>% 
  sf::st_transform(crs = 4326) 

years <- 2017:2023
#kwb.utils::hsOpenWindowsExplorer(exp_dir)

exp_dir <- fs::path_join(c(getwd(), "../vignettes/gee/lakes-berlin_point-malte")) %>%  fs::path_abs()
...
lakes_malte_points_parallel <- kwb.satellite::gee_get_data_for_years_parallel(
  ...
  spatial_fun = "mean",
  col_lakename = "GEW_NAME",
  col_lakeid = "GEW_NR_NEU",
  debug_dir = exp_dir,
  export_dir = exp_dir,
  debug = TRUE)
)

lakes_selected <- lakes_berlin  %>% 
  dplyr::filter(SEE_KZ %in% lakes_malte$SCHLUESSEL)

exp_dir <- fs::path_join(c(getwd(), "../vignettes/gee/lakes-berlin_point-on-surface")) %>% fs::path_abs()
fs::dir_create(exp_dir)
...
exp_dir <- fs::path_join(c(getwd(), "../vignettes/gee/lakes-berlin_polygon")) %>% fs::path_abs()
fs::dir_create(exp_dir)
